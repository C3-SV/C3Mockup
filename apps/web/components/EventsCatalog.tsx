...
export default function EventsCatalog({ initialEvents }: EventsCatalogProps) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (activeFilter === "historicos") {
      return initialEvents.filter(
        (event) => event.status === "Histórico" || event.status === "Apoyo institucional",
      );
    }
    return initialEvents.filter((event) => event.lines.includes(activeFilter));
